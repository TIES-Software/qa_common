...
class child_element_to_be_present_in_element(object):
    """ An expectation for checking if the given element is present in the
    parent element.
    Custom function for TIES
    locator, parent_element
    """
    ...
    def __call__(self, driver):
        try:
            child_element = self.parent.find_element(*self.locator)
            return child_element if child_element.is_enabled() else False
        except StaleElementReferenceException:
            return False
